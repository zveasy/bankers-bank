import os
from typing import Callable

import pytest
from fastapi.testclient import TestClient

from mocks.mock_finastra_api import app as mock_app
from sdk.python.bankersbank.client import BankersBankClient
from tests.test_helpers import REQUESTS_AVAILABLE

pytestmark = pytest.mark.skipif(not REQUESTS_AVAILABLE, reason="requests not installed")


def _patch_requests(monkeypatch: "pytest.MonkeyPatch", client: TestClient):
    """Route requests.* calls through FastAPI TestClient."""

    def _req(method: str, url: str, **kwargs):
        # Strip host so TestClient handles the path
        path = url.split("testserver")[-1]
        if not path:
            path = "/"
        return client.request(method.upper(), path, **kwargs)

    monkeypatch.setattr("requests.request", _req, raising=True)
    monkeypatch.setattr(
        "requests.get", lambda url, **kw: _req("GET", url, **kw), raising=True
    )
    monkeypatch.setattr(
        "requests.post", lambda url, **kw: _req("POST", url, **kw), raising=True
    )


if REQUESTS_AVAILABLE:
    import requests


def reset_collateral(client):
    # Reset the in-memory collateral registry for test isolation
    resp = client.post("/collateral/reset")
    assert resp.status_code == 200


def setup_collateral_and_account(
    client, account_id, address="123 Main St", valuation=100000
):
    # Add collateral
    collateral_data = {
        "address": address,
        "valuation": valuation,
        "owner": "Alice",
        "title_status": "clear",
    }
    client.add_collateral(collateral_data)
    # No need to create account, just ensure balance exists in mock


@pytest.mark.enable_socket
def test_calculate_ltv_against_mock(monkeypatch):
    os.environ["USE_MOCK_BALANCES"] = "true"
    with TestClient(mock_app) as http_client:
        base_url = str(http_client.base_url)
        _patch_requests(monkeypatch, http_client)
        reset_collateral(http_client)
        client_sdk = BankersBankClient(base_url, token="dummy")
        account_id = "456783434"  # This is in SAMPLE_ACCOUNTS and SAMPLE_BALANCES
        setup_collateral_and_account(client_sdk, account_id)
        result = client_sdk.calculate_ltv(account_id)
        assert result["ltv"] == 10500.0 / 100000


@pytest.mark.enable_socket
def test_ltv_zero_collateral(monkeypatch):
    os.environ["USE_MOCK_BALANCES"] = "true"
    with TestClient(mock_app) as http_client:
        base_url = str(http_client.base_url)
        _patch_requests(monkeypatch, http_client)
        reset_collateral(http_client)
        client_sdk = BankersBankClient(base_url, token="dummy")
        account_id = "456783434"
        # Do not add collateral
        with pytest.raises(ValueError, match="Total collateral valuation is zero"):
            client_sdk.calculate_ltv(account_id)


@pytest.mark.enable_socket
def test_ltv_multiple_collateral(monkeypatch):
    os.environ["USE_MOCK_BALANCES"] = "true"
    with TestClient(mock_app) as http_client:
        base_url = str(http_client.base_url)
        _patch_requests(monkeypatch, http_client)
        reset_collateral(http_client)
        client_sdk = BankersBankClient(base_url, token="dummy")
        account_id = "456783434"
        setup_collateral_and_account(
            client_sdk, account_id, address="1 A St", valuation=100000
        )
        setup_collateral_and_account(
            client_sdk, account_id, address="2 B St", valuation=50000
        )
        result = client_sdk.calculate_ltv(account_id)
        assert result["total_collateral"] == 150000
        assert result["ltv"] == 10500.0 / 150000


def test_ltv_no_booked_balance(monkeypatch):
    os.environ["USE_MOCK_BALANCES"] = "true"
    with TestClient(mock_app) as http_client:
        base_url = str(http_client.base_url)
        _patch_requests(monkeypatch, http_client)
        reset_collateral(http_client)
        client_sdk = BankersBankClient(base_url, token="dummy")
        # Use a non-existent account id
        account_id = "999999999"
        setup_collateral_and_account(client_sdk, account_id)
        import httpx

        with pytest.raises(httpx.HTTPStatusError):
            client_sdk.calculate_ltv(account_id)
