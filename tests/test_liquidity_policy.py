...
from quantengine.liquidity.policy import (LiquidityPolicy, compute_buffer,
                                          evaluate, is_holiday)
...
@pytest.mark.parametrize(
    ...
    [
        # Happy path – ok
        (
            {"min_cash_bps": 500, "max_draw_bps": 10_000},
            1_000_000,
            100_000,
            _dt.date(2025, 1, 15),
            [],
        ...
        # Holiday violation
        (
            {"settlement_calendar": {"2025-12-25"}},
            1_000_000,
            100_000,
            _dt.date(2025, 12, 25),
            ["holiday"],
...
def test_evaluate(policy_kwargs, available, draw, asof, reasons, allowed):
    ...
    res = evaluate(
        policy, available_cash_usd=available, requested_draw_usd=draw, asof=asof
    )
