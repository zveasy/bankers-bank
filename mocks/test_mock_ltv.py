...
def test_ltv_calculate_non_numeric():
    response = client.post("/ltv/calculate", json={"collateral_value": "abc", "loan_amount": 50000})
    assert response.status_code == 400
    assert "Values must be numeric" in response.text

def test_ltv_calculate_zero_collateral():
    response = client.post("/ltv/calculate", json={"collateral_value": 0, "loan_amount": 50000})
    assert response.status_code == 400
