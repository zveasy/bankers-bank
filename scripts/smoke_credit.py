...
def main() -> None:
    ...
    r1 = requests.post(
        f"{BASE}/api/credit/v1/draw", json=draw_body, headers=headers, timeout=TIMEOUT
    )
    r1.raise_for_status()
    ...
    if r2.status_code not in (200, 201):
        print(
            f"Unexpected status for idempotent replay: {r2.status_code}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    assert (
        "available" in status and "outstanding" in status
    ), "Status response missing keys"

    metrics_text = requests.get(f"{BASE}/metrics", timeout=TIMEOUT).text
    if (
        "credit_actions_total" not in metrics_text
        or "credit_provider_latency_seconds_bucket" not in metrics_text
    ):
