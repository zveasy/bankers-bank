...
class AssetSnapshot(SQLModel, table=True):
    ...
    eligibleCollateralUSD: float = Field(default=0.0, sa_column=Column("eligiblecollateralusd", Float, nullable=False))
    totalBalancesUSD: float = Field(default=0.0, sa_column=Column("totalbalancesusd", Float, nullable=False))
    undrawnCreditUSD: float = Field(default=0.0, sa_column=Column("undrawncreditusd", Float, nullable=False))
