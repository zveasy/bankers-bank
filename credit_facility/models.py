...
from sqlalchemy import Column, DateTime, Float, String, UniqueConstraint, Index
...
class CreditDraw(SQLModel, table=True):
    ...
    idempotency_key: str = Field(sa_column=Column("idempotency_key", String, nullable=False))
    bank_id: str = Field(sa_column=Column("bank_id", String, nullable=False))
    amount: float = Field(sa_column=Column("amount", Float, nullable=False))
    currency: str = Field(sa_column=Column("currency", String, nullable=False, default="USD"))
    ts: datetime = Field(default_factory=datetime.utcnow, sa_column=Column("ts", DateTime, nullable=False))

    status: str = Field(sa_column=Column("status", String, nullable=False, default="pending"))
    provider: Optional[str] = Field(default=None, sa_column=Column("provider", String))

    __table_args__ = (
        UniqueConstraint("idempotency_key", "bank_id", name="credit_draw_idem_uniq"),
        Index("ix_credit_draw_bank_ts", "bank_id", "ts"),
    )
...
class CreditRepayment(SQLModel, table=True):
    ...
    __table_args__ = (
        Index("ix_credit_repay_bank_ts", "bank_id", "ts"),
    )
...
class ProviderStatus(SQLModel, table=True):
    ...
    last_success: Optional[datetime] = Field(default=None, sa_column=Column("last_success", DateTime))
    last_error: Optional[datetime] = Field(default=None, sa_column=Column("last_error", DateTime))
    message: Optional[str] = Field(default=None, sa_column=Column("message", String))

    __table_args__ = (
        UniqueConstraint("provider", name="provider_status_uniq"),
    )
